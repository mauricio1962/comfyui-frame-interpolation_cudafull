...
def postprocess_frames(frames, keep_on_gpu=False):
    """后处理帧数据，可选择保持在GPU上避免显存交换"""
    processed = einops.rearrange(frames, "n c h w -> n h w c")[..., :3]
    # 🔥 只有在明确要求时才转到CPU，避免显存交换
    return processed if keep_on_gpu else processed.cpu()
...
def _generic_frame_loop(
    ...
    # 🔥 GPU优化：整个过程保持在GPU上，避免CPU交换
    if not frames.is_cuda:
        frames = frames.to(DEVICE)
    
    # 预分配GPU内存
    output_frames = torch.empty(multiplier*frames.shape[0], *frames.shape[1:], dtype=dtype, device=DEVICE)
    out_len = 0
    ...
    for frame_itr in range(len(frames) - 1): # Skip the final frame since there are no frames after it
        ...
        output_frames[out_len] = frame0.to(dtype=dtype) # 直接在GPU上存储
        out_len += 1
        ...
        if use_timestep:
            ...
            # 🔥 关键修复：保持在GPU上，不转CPU！
            middle_frame_batches.extend([mf.to(dtype=dtype) for mf in middle_frames])
        
        # Copy middle frames to output (保持在GPU)
        ...
        # 减少缓存清理频率，避免打断GPU流水线
        if number_of_frames_processed_since_last_cleared_cuda_cache >= clear_cache_after_n_frames * 2:
            if final_logging:
                print("Comfy-VFI: GPU缓存清理...", end=' ')
            soft_empty_cache()
            number_of_frames_processed_since_last_cleared_cuda_cache = 0
            if final_logging:
                print("完成")
    ...
    if final_logging:
        print(f"Comfy-VFI: GPU上完成! 生成{out_len}帧，分辨率{output_frames[0].shape}")
    # Append final frame (保持在GPU)
    output_frames[out_len] = frames[-1:].to(dtype=dtype)
    out_len += 1
    
    # Final cache clear
    if final_logging:
        print("Comfy-VFI: 最终缓存清理...", end=' ')
    soft_empty_cache()
    if final_logging:
        print("完成")
    
    # 🔥 只在最后统一转CPU，避免频繁交换
    return output_frames[:out_len].cpu()
